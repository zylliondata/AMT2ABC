...
> `inventory-query` · category: `form` · industry: `general`

库存查询 Module:提供筛选区(关键词、批次号、物料类型、库位、区域、状态)+ 查询/重置按钮 + 分页表格,支持行选中事件。属于纯查询能力,不含写操作。
...
- **`bodyParams.table`**:请求体中的 `table` 字段(值为 `pet_inventory_items`)是数据表的结构性 key,不携带敏感数据。
...
| 检查点 | 原始值 | 脱敏后 |
|--------|--------|--------|
| `applicationInfo.id` | `6a650854...`(24 位内部库 id) | `000000000000000000000000` |
| `applicationInfo.createAt` | `1780390134614`(真实时间戳) | `0` |
| `applicationInfo.name` | `abc_inventory_query_v2` | `inventory-query` |
...
| `js_autoSearch` 脚本 | 内部 API fetch(`/api/fabric/resource/info`、内部系统名、内部 cookie 名) | 精简为仅按注入的 `resourceId` 自动查询 |
